...
return message.reply("**لا يمكنك تغيير الاسم يجب عليك الانتظآر لمدة ساعتين . **");
  ...
  if (message.content.startsWith(adminprefix + 'avatar1')) {
            ...
            ctx.drawImage(ground, 0, 0, 800, 300);
                ...
                jimp.read(url, (err, ava) => {
                    ...
                    ava.getBuffer(jimp.MIME_PNG, (err, buf) => {
                        ...
                        ctx.fillText(`welcome to ${member.guild.name}`, 380, 222);
                        
                        ctx.font = "bold 50px Arial";
                        ctx.fontSize = '35px';
                        ctx.fillStyle = "#f1f1f1";
                        ctx.textAlign = "center";
                        ctx.fillText(member.user.username, 470, 180);
 
                let Avatar = Canvas.Image;
                              let ava = new Avatar;
                              ava.src = buf;
                              ctx.beginPath();
                              ctx.arc(100, 470, 220, 0, Math.PI*2);
                              ctx.stroke();
                                 ctx.clip();
...
client.on('message', ( message ) => {
  ...
  if(message.channel.id !== '426506936683331584') return;
...
client.on('ready',async () => {
  let GUILDID = '426500762453934080'; // اي دي السيرفر
  let CHANNELID = '459313784104943616'; // اي دي الروم
